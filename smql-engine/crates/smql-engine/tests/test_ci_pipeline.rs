...
fn cascade_cmd(instance_id: &str, to_state: &str) -> TransitionCommand {
...
async fn job_lifecycle_pass() {
    ...
    let r = engine.transition(&transition_cmd(&jid, "running")).await.unwrap();
    assert_eq!(r.to_state, "running");

    let r = engine.transition(&transition_cmd(&jid, "passed")).await.unwrap();
...
async fn job_lifecycle_fail() {
    ...
    engine.transition(&transition_cmd(&jid, "running")).await.unwrap();
    let r = engine.transition(&transition_cmd(&jid, "failed")).await.unwrap();
...
async fn stage_passes_when_all_jobs_pass() {
    ...
    // Start stage
    engine.transition(&transition_cmd(&sid, "running")).await.unwrap();

    // Run jobs
    engine.transition(&transition_cmd(&j1, "running")).await.unwrap();
    engine.transition(&transition_cmd(&j2, "running")).await.unwrap();

    // Pass only one job — stage should NOT pass
    engine.transition(&transition_cmd(&j1, "passed")).await.unwrap();
    let result = engine.transition(&transition_cmd(&sid, "passed")).await;
    assert!(result.is_err(), "stage should not pass: not ALL jobs passed");

    // Pass second job
    engine.transition(&transition_cmd(&j2, "passed")).await.unwrap();

    // Now stage should pass
    let r = engine.transition(&transition_cmd(&sid, "passed")).await.unwrap();
    assert_eq!(r.to_state, "passed");
}
...
async fn stage_fails_when_any_job_fails() {
    ...
    engine.transition(&transition_cmd(&j1, "running")).await.unwrap();
    engine.transition(&transition_cmd(&j1, "failed")).await.unwrap();

    // Stage should fail because ANY(jobs, STATE IS failed)
    let r = engine.transition(&transition_cmd(&sid, "failed")).await.unwrap();
    assert_eq!(r.to_state, "failed");
}
...
async fn pipeline_passes_when_all_stages_pass() {
    ...
    // Start pipeline
    engine.transition(&transition_cmd(&pid, "running")).await.unwrap();

    // Run and pass all stages
    engine.transition(&transition_cmd(&s1, "running")).await.unwrap();
    engine.transition(&transition_cmd(&j1, "running")).await.unwrap();
    engine.transition(&transition_cmd(&j1, "passed")).await.unwrap();
    engine.transition(&transition_cmd(&s1, "passed")).await.unwrap();

    engine.transition(&transition_cmd(&s2, "running")).await.unwrap();
    engine.transition(&transition_cmd(&j2, "running")).await.unwrap();
    engine.transition(&transition_cmd(&j2, "passed")).await.unwrap();
    engine.transition(&transition_cmd(&s2, "passed")).await.unwrap();

    // Pipeline should pass
    let r = engine.transition(&transition_cmd(&pid, "passed")).await.unwrap();
    assert_eq!(r.to_state, "passed");
}
...
async fn pipeline_fails_when_any_stage_fails() {
    ...
    engine.transition(&transition_cmd(&s2, "running")).await.unwrap();
    engine.transition(&transition_cmd(&j2, "running")).await.unwrap();
    engine.transition(&transition_cmd(&j2, "failed")).await.unwrap();
    engine.transition(&transition_cmd(&s2, "failed")).await.unwrap();

    // Pipeline should fail
    let r = engine.transition(&transition_cmd(&pid, "failed")).await.unwrap();
    assert_eq!(r.to_state, "failed");
}
...
async fn cascade_cancel_three_levels() {
    ...
    let r = engine.transition(&cascade_cmd(&pid, "cancelled")).await.unwrap();
...
async fn stage_skip() {
    ...
    let r = engine.transition(&transition_cmd(&sid, "skipped")).await.unwrap();
...
async fn pipeline_cannot_pass_without_all_stages() {
    ...
    let result = engine.transition(&transition_cmd(&pid, "passed")).await;
